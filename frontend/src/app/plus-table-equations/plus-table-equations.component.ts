...
import { AnswerDialogPlusEquationsComponent } from '../answer-dialog-plus-equations/answer-dialog-plus-equations.component';
import { API_BASE_URL } from '../api.config';

@Component({
  selector: 'app-plus-table-equations',
  standalone: true,
  templateUrl: './plus-table-equations.component.html',
  styleUrl: './plus-table-equations.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [AnswerDialogPlusEquationsComponent]
})
export class PlusTableEquationsComponent implements OnInit, OnDestroy {
  ...
  private async sendAnswer(questionId: number, correct: boolean): Promise<void> {
    ...
    try {
      await fetch(`${this.apiBase}/plustable-equations/answer?${params.toString()}`, {
        method: 'POST'
      });
    } catch (error) {
  ...
  private async loadProgress(childId: string): Promise<void> {
    ...
    try {
      const response = await fetch(`${this.apiBase}/plustable-equations/progress/${childId}`);
      if (!response.ok) {
        return;
      }
