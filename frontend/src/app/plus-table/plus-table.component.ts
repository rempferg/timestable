...
import { AnswerDialogPlusComponent } from '../answer-dialog-plus/answer-dialog-plus.component';
import { API_BASE_URL } from '../api.config';

@Component({
  selector: 'app-plus-table',
  standalone: true,
  templateUrl: './plus-table.component.html',
  styleUrl: './plus-table.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [AnswerDialogPlusComponent]
})
export class PlusTableComponent implements OnInit, OnDestroy {
  ...
  private async sendAnswer(questionId: number, correct: boolean): Promise<void> {
    ...
    try {
      await fetch(`${this.apiBase}/plustable/answer?${params.toString()}`, {
        method: 'POST'
      });
    } catch (error) {
  ...
  private async loadProgress(childId: string): Promise<void> {
    ...
    try {
      const response = await fetch(`${this.apiBase}/plustable/progress/${childId}`);
      if (!response.ok) {
        return;
      }
