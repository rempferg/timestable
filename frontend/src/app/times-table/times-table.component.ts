...
export class TimesTableComponent implements OnInit, OnDestroy {
  ...
  private async sendAnswer(questionId: number, correct: boolean): Promise<void> {
    ...
    try {
      await fetch(`${this.apiBase}/timestable/answer?${params.toString()}`, {
        method: 'POST'
      });
    } catch (error) {
  ...
  private async loadProgress(childId: string): Promise<void> {
    ...
    try {
      const response = await fetch(`${this.apiBase}/timestable/progress/${childId}`);
      if (!response.ok) {
        return;
      }
